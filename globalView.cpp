...
{
    epsilon_ = pos * ui->doubleSpinBox_epsilon->singleStep();
    ui->doubleSpinBox_epsilon->setValue(epsilon_);
    qDebug() << "epsilon_: " << epsilon_;
}

void GlobalView::on_doubleSpinBox_epsilon_valueChanged(double pos)
{
    epsilon_ = pos;
    ui->slider_epsilon->setValue(pos / ui->doubleSpinBox_epsilon->singleStep());
}

void GlobalView::on_slider_delta_sliderMoved(int pos)
{
    delta_ = pos * ui->doubleSpinBox_delta->singleStep();
    ui->doubleSpinBox_delta->setValue(delta_);
    qDebug() << "delta_: " << delta_;
}

void GlobalView::on_doubleSpinBox_delta_valueChanged(double pos)
{
    delta_ = pos;
    ui->slider_delta->setValue(pos / ui->doubleSpinBox_delta->singleStep());
}
