...
#                                 continue
#                             noise_op = ops[j]
#                             random_circuit += noise_op.on(qubit)
...
#         final_circuit.append(random_circuit)
#         print('random_kraus.shape =', np.array(kraus_).shape)
#     return Circuit(final_circuit[0]), np.array(kraus_)


def random_insert_ops(origin_circuit, nums_and_ops, with_ctrl=True):
    """Insert single-qubit operators at random points in the circuit.
    ...
    #  筛选线路中所有非测量门的位置，并存入列表 aval_indexes，以确定可以插入噪声的合法位置
    aval_indexes = []
    for i, gate in enumerate(origin_circuit):
        if not isinstance(gate, Measure):
            aval_indexes.append(i)
    ...
    for i in range(len(nums_and_ops)):
        if len(nums_and_ops[i]) != 2:
            raise ValueError(
                f'The format of the argment "nums_and_ops" should be "[num_0, op_0], [num_1, op_1], ....".')
        if nums_and_ops[i][0] > len(aval_indexes):
            raise ValueError(
                f'The number of positions allowed to insert channel should be less than {len(aval_indexes)}, but get {nums_and_ops[i][0]}.')
        nums.append(nums_and_ops[i][0])
        ops.append(nums_and_ops[i][1])

    indexes = []
    for num in nums:
        tem = sorted(np.random.choice(aval_indexes, size=num, replace=False))
        indexes.append(tem)

    random_circuit = Circuit()
    qubits_num = origin_circuit.n_qubits
    kraus_ = [np.eye(2 ** qubits_num)]  # U
    selected_qubits = []
    for i, gate in enumerate(origin_circuit):
        if isinstance(gate, Measure):
            continue
            ...
            for k in tem_indexs:
                if k == i:
                    ...
                    if qubit not in selected_qubits:  # the `qubit` has been selected.
                        ...
                        # print('kraus_.shape =', np.array(kraus_).shape)
    # print('random_kraus.shape =', np.array(kraus_).shape)
    return random_circuit, np.array(kraus_)


def generate_circuit_with_random_noise(circ, model_name_, to_save_figure=False, file_path=''):
    ...
    if to_save_figure:
        if file_path != '':
            circ.svg().to_file(file_path)  # qasm_file chop '.qasm'
...
def generate_circuit_with_specified_noise(circ, origin_kraus_, noise, noise_list_, kraus_file_, noise_p_: float,
                                            model_name_, to_save_figure=False, file_path=''):
    all_measures = []
    for gate in circ:
        if isinstance(gate, Measure):
            all_measures.append(gate)
