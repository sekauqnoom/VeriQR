...
if '.npz' in str(argv[1]):
    ...
    circuit, random_kraus = generate_circuit_with_random_noise(circuit, model_name, True)
    ...
    if arg_num > 8:
        ...
        final_kraus, noise_name = generate_circuit_with_specified_noise(circuit, random_kraus, noise_type, noise_list,
    ...
    for kraus_temp in [kraus, random_kraus, final_kraus]:
        if GET_NEW_DATASET:
            if 'mnist' in data_file:
                ...
                acc, time, _, new_data, new_labels = verifier(kraus_temp, O, data, label, c_eps, type,
                                                              GET_NEW_DATASET, origin_dataset_size)
            np.savez('./model_and_data/newdata_for_AT/{}_c2_{}_{}_by_{}.npz'.format(model_name,
                                                                                    noise_name, noise_p, c_eps),
