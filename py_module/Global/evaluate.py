...
def batch_evaluate(model):
    if '.qasm' in model:  # model is qasm_file
        ...
        random_mq_circuit, random_cirq_circuit = generate_circuit_with_random_noise(origin_mq_circuit_,
    ...
    with (open("./results/global_results.csv", "a+") as csvfile):
        ...
        try:
            with time_limit():
                ...
                start = time.time()
                res = 'YES' if verify(origin_k, epsilon, delta) else 'NO'
        ...
        try:
            with time_limit():
                ...
                start = time.time()
                res = 'YES' if verify(random_k, epsilon, delta) else 'NO'
        ...
        for noise_type in noise_types:
            ...
            for _ in range(3):
                ...
                final_mq_circuit, final_cirq_circuit, _ = generate_circuit_with_specified_noise(random_mq_circuit_,
                                                                                                random_cirq_circuit_,
                ...
                try:
                    with time_limit():
                        ...
                        start = time.time()
                        res = 'YES' if verify(final_k, epsilon, delta) else 'NO'
...
def batch_evaluate_for_plot(model):
    if '.qasm' in model:  # model is qasm_file
        ...
        model_name = qasm_file[qasm_file.rfind("/") + 1:-5]
        origin_mq_circuit, origin_cirq_circuit, cirq_qubits = get_origin_circuit(qasm_file)
        random_mq_circuit, random_cirq_circuit = generate_circuit_with_random_noise(origin_mq_circuit,
